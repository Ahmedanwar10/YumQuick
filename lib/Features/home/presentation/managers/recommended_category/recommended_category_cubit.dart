...
class RecommendedCategoryCubit extends Cubit<RecommendedCategoryState> {
  ...
  RecommendedCategoryCubit(this.recommendedCategoryRepo)
      : super(RecommendedCategoryInitial());
  ...
  Future<void> getRecommendedCategory() async {
    ...
    result.fold(
      (failure) => emit(
          RecommendedCategoryFailureState(errMessage: failure.errorMessage)),
      (recommendedCategory) =>
          emit(RecommendedCategorySuccessState([recommendedCategory])),
    );
  }
}
